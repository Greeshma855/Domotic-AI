...
const Sidebar = ({ isExpanded, onToggle, onLogout }) => {
  ...
  return (
    <motion.div
      initial={false}
      animate={{ width: isExpanded ? "16rem" : "4rem" }}
      className="fixed top-0 left-0 h-screen shadow-lg bg-gray-900 backdrop-blur-sm border-r border-white/20 overflow-hidden"
    >
      <div className="h-full flex flex-col justify-between">
        <div>
          <div className="p-4 flex items-center justify-between">
            <motion.div
              ...
              animate={{ gap: isExpanded ? "0.5rem" : "0" }}
              className="flex items-center cursor-pointer"
              onClick={onToggle} // Toggle when "D" is clicked
            >
              <div className="w-8 h-8 bg-gradient-to-r from-violet-600 to-indigo-600 rounded-lg flex items-center justify-center flex-shrink-0">
                <span className="text-white font-bold">D</span>
              </div>
              ...
              <nav className="space-y-1 w-full">
                {menuItems.map((item) => (
                  <motion.a
                    ...
                    // } transition-all duration-300`}
                    className={`flex items-center ${
                    isExpanded ? "space-x-2" : "w-11 justify-center"
